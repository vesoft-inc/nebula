...
TEST(AddEdgesTest, VersionTest) {
    ...
    auto checkEdgeByPrefix = [&](PartitionID partId, VertexID vertexId,
        ...
        auto prefix = NebulaKeyUtils::edgePrefix(partId, vertexId, edge);
        std::unique_ptr<kvstore::KVIterator> iter;
        EXPECT_EQ(kvstore::ResultCode::SUCCEEDED, kv->prefix(0, partId, prefix, &iter));
        int num = 0;
        while (iter->valid()) {
...

    {
        ...
        checkEdgeByRange(partitionId, srcId, srcId*100 + 1, srcId*100 + 2,
                         srcId*100 + 3, 10000, 10000);
    }

    {
        ...
        checkEdgeByRange(partitionId, srcId, srcId*100 + 1, srcId*100 + 2, srcId*100 + 3, 10000, 1);
    }
}
