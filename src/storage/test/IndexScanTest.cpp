...
static std::unique_ptr<meta::SchemaManager> mockSchemaMan(GraphSpaceID spaceId,
  ...
  schemaMan->addEdgeSchema(
      spaceId /*space id*/, edgeType /*edge type*/, genEdgeSchemaProvider(10, 10));
  schemaMan->addTagSchema(spaceId /*space id*/, tagId, genTagSchemaProvider(tagId, 3, 3));
...
void mockData(kvstore::KVStore* kv,
  ...
  for (auto partId = 0; partId < 3; partId++) {
    ...
    for (auto vertexId = partId * 10; vertexId < (partId + 1) * 10; vertexId++) {
      ...
      auto indexKey = genVertexIndexKey(schemaMan, val, spaceId, partId, tagId, vindex, vertexId);
      ...
      for (auto dstId = 10001; dstId <= 10007; dstId++) {
        ...
        auto ekey = NebulaKeyUtils::edgeKey(partId, vertexId, edgeType, 0, dstId);
        ...
        auto edgeIndex =
            genEdgeIndexKey(schemaMan, eval, spaceId, partId, edgeType, eindex, vertexId, dstId);
        data.emplace_back(std::move(edgeIndex), "");
        data.emplace_back(std::move(ekey), std::move(eval));
      }
    }
    folly::Baton<true, std::atomic> baton;
    kv->asyncMultiPut(0, partId, std::move(data), [&](cpp2::ErrorCode code) {
...
static cpp2::LookUpVertexIndexResp execLookupVertices(const std::string& filter,
  ...
  auto* processor =
      LookUpVertexIndexProcessor::instance(kv.get(), schemaMan.get(), indexMan.get(), nullptr);
...
static cpp2::LookUpEdgeIndexResp execLookupEdges(const std::string& filter,
  ...
  mockData(kv.get(), schemaMan.get(), indexMan.get(), type, tagId, spaceId);
  auto* processor =
      LookUpEdgeIndexProcessor::instance(kv.get(), schemaMan.get(), indexMan.get(), nullptr);
  cpp2::LookUpIndexRequest req;
  decltype(req.parts) parts;
  parts.emplace_back(0);
  parts.emplace_back(1);
...
TEST(IndexScanTest, SimpleScanTest) {
  {
    ...
    auto relExp =
        std::make_unique<RelationalExpression>(aliaExp, RelationalExpression::Operator::EQ, priExp);
...
TEST(IndexScanTest, AccurateScanTest) {
  {
    ...
    auto* r1 = new RelationalExpression(ape0, RelationalExpression::Operator::EQ, pe0);
    ...
    auto* r2 = new RelationalExpression(ape1, RelationalExpression::Operator::EQ, pe1);
    ...
    auto* r3 = new RelationalExpression(ape2, RelationalExpression::Operator::EQ, pe2);
    auto* le1 = new LogicalExpression(r1, LogicalExpression::AND, r2);

    auto logExp = std::make_unique<LogicalExpression>(le1, LogicalExpression::AND, r3);
...
TEST(IndexScanTest, SeekScanTest) {
  {
    ...
    auto r1 = std::make_unique<RelationalExpression>(ape0, RelationalExpression::Operator::GT, pe0);
...
TEST(IndexScanTest, PrefixScanTest) {
  {
    ...
    auto* r2 = new RelationalExpression(ape1, RelationalExpression::Operator::GT, pe1);

    auto logExp = std::make_unique<LogicalExpression>(r1, LogicalExpression::AND, r2);
    auto resp = execLookupVertices(Expression::encode(logExp.get()));
    EXPECT_EQ(0, resp.result.failed_codes.size());
    EXPECT_EQ(4, resp.get_schema()->get_columns().size());
    EXPECT_EQ(30, resp.rows.size());
...
TEST(IndexScanTest, EdgeStringTest) {
  {
    ...
    auto r1 = std::make_unique<RelationalExpression>(ape0, RelationalExpression::Operator::EQ, pe0);
