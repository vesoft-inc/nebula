...
std::vector<meta::cpp2::ColumnDef> genColDefs(const std::vector<meta::cpp2::PropertyType>& types) {
  auto N = types.size();
...
int main(int argc, char** argv) {
  ...
  for (auto& item : gAddedEdges) {
    ...
    LOG(INFO) << folly::sformat("testId={}, testCalled={}, cnt={}", item.first, item.second, cnt);
  }
}
