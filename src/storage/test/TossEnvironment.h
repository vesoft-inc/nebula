...
struct TossEnvironment {
  ...
  std::unique_ptr<meta::MetaClient> setupMetaClient(const std::string& metaName,
                                                    uint32_t metaPort) {
  ...
  void init(const std::string& spaceName,
    ...
    while (!leaderLoaded) {
      ...
      for (auto& leader : leaderMap) {
        LOG(INFO) << "spaceId=" << leader.first.first << ", part=" << leader.first.second
                  << ", host=" << leader.second;
  ...
  int setupSpace(const std::string& spaceName, int nPart, int nReplica) {
    ...
    if (!fCreateSpace.value().ok()) {
      LOG(FATAL) << "!fCreateSpace.value().ok(): " << fCreateSpace.value().status().toString();
    }
  ...
  folly::SemiFuture<StorageRpcResponse<cpp2::ExecResponse>> addEdgesAsync(
      const std::vector<cpp2::NewEdge>& edges, bool useToss = true) {
    auto propNames = makeColNames(edges.back().get_props().size());
    return sClient_->addEdges(spaceId_, 0, 0, edges, propNames, true, nullptr, useToss);
  }
  ...
  std::vector<Value> getProps(cpp2::NewEdge edge) {
    ...
    // auto sDst = std::string(reinterpret_cast<const
    // char*>(&edge.key.dst.getInt()), 8);
  ...
  folly::SemiFuture<StorageRpcResponse<cpp2::GetNeighborsResponse>> getNeighborsWrapper(
   ...
   * @param extraSameKey
   *        if set, gen another edge, has the same src & dst will always equal
   * to src + 1
   */
