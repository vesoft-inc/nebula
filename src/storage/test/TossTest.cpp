...
class TossTest : public ::testing::Test {
 ...
 protected:
  ...
  std::vector<meta::cpp2::ColumnDef> genColDefs(
...
TEST_F(TossTest, neighbors_test_3) {
  ...
  decltype(strProps) expect{edges[0].get_props()[1].toString(), lockEdge.get_props()[1].toString()};
...
TEST_F(TossTest, neighbors_test_5) {
  ...
  decltype(actualProps) expect{edges[0].get_props()[1].toString(),
...
TEST_F(TossTest, neighbors_test_6) {
  ...
  // step 4th: the get neighbors result is from lock
  auto actualProps = env_->extractStrVals(svec);
  decltype(actualProps) expect{lockEdge.get_props()[1].toString(),
                               edges[1].get_props()[1].toString()};
  ASSERT_EQ(actualProps, expect);
...
TEST_F(TossTest, neighbors_test_10) {
  ...
  // step 4th: the get neighbors result is from lock
  auto actualProps = env_->extractStrVals(svec);
  decltype(actualProps) expect{lock0.get_props()[1].toString(), edges[1].get_props()[1].toString()};
  ASSERT_EQ(actualProps, expect);
...
TEST_F(TossTest, neighbors_test_18) {
  ...
  auto svec = TossTestUtils::splitNeiResults(props);
  auto actualProps = env_->extractStrVals(svec);
  decltype(actualProps) expect{edges[0].get_props()[1].toString(), lock1.get_props()[1].toString()};
  ASSERT_EQ(actualProps, expect);
