...
#include "common/thrift/ThriftTypes.h"               // for VertexID, EdgeRa...
...
TEST(UpdateEdgeTest, No_Filter_Test) {
  ...
  EXPECT_TRUE(mockEdgeData(env, parts, spaceVidLen));
  {
    ...
    req.updated_props_ref() = std::move(props);
...
TEST(UpdateEdgeTest, CorruptDataTest) {
  ...
  folly::Baton<> baton;
  env->kvstore_->asyncMultiPut(spaceId, partId, std::move(data), [&](nebula::cpp2::ErrorCode code) {
    ASSERT_EQ(code, nebula::cpp2::ErrorCode::SUCCEEDED);
    baton.post();
  });
  baton.wait();
