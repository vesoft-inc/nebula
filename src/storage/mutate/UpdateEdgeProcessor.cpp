...
void UpdateEdgeProcessor::doProcess(const cpp2::UpdateEdgeRequest& req) {
  ...
  if (!NebulaKeyUtils::isValidVidLen(
    ...
    LOG(ERROR) << "Space " << spaceId_ << ", vertex length invalid, "
               << " space vid len: " << spaceVidLen_ << ",  edge srcVid: " << edgeKey_.get_src()
...
StoragePlan<cpp2::EdgeKey> UpdateEdgeProcessor::buildPlan(nebula::DataSet* result) {
  ...
  auto filterNode = std::make_unique<FilterNode<cpp2::EdgeKey>>(
      context_.get(), edgeUpdate.get(), expCtx_.get(), filterExp_);
  ...
  auto resultNode = std::make_unique<UpdateResNode<cpp2::EdgeKey>>(
...
nebula::cpp2::ErrorCode UpdateEdgeProcessor::buildEdgeContext(const cpp2::UpdateEdgeRequest& req) {
  ...
  if (edgeContext_.edgeNames_.size() != 1 || iter == edgeContext_.edgeNames_.end()) {
  ...
  if (expCtx_ == nullptr) {
    expCtx_ = std::make_unique<StorageExpressionContext>(
        spaceVidLen_, isIntId_, context_->edgeName_, context_->edgeSchema_, true);
  }

  return nebula::cpp2::ErrorCode::SUCCEEDED;
}

void UpdateEdgeProcessor::onProcessFinished() { resp_.props_ref() = std::move(resultDataSet_); }
