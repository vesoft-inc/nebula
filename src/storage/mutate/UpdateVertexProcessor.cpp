...
#include "common/expression/PropertyExpression.h"  // for SourcePropertyExpr...
...
#include "storage/exec/UpdateNode.h"               // for UpdateTagNode
...
void UpdateVertexProcessor::doProcess(const cpp2::UpdateVertexRequest& req) {
  ...
  VLOG(3) << "Update vertex, spaceId: " << spaceId_ << ", partId: " << partId << ", vId: " << vId;
...
nebula::cpp2::ErrorCode UpdateVertexProcessor::checkAndBuildContexts(
...
StoragePlan<VertexID> UpdateVertexProcessor::buildPlan(nebula::DataSet* result) {
  ...
  auto filterNode = std::make_unique<FilterNode<VertexID>>(
      context_.get(), tagUpdate.get(), expCtx_.get(), filterExp_);
  ...
  auto resultNode = std::make_unique<UpdateResNode<VertexID>>(
      context_.get(), updateNode.get(), getReturnPropsExp(), expCtx_.get(), result);
...
nebula::cpp2::ErrorCode UpdateVertexProcessor::buildTagContext(
    const cpp2::UpdateVertexRequest& req) {
  ...
  if (tagContext_.tagNames_.size() != 1 || iter == tagContext_.tagNames_.end()) {
  ...
  if (expCtx_ == nullptr) {
    expCtx_ = std::make_unique<StorageExpressionContext>(
        spaceVidLen_, isIntId_, context_->tagName_, context_->tagSchema_, false);
  }
  return nebula::cpp2::ErrorCode::SUCCEEDED;
}
