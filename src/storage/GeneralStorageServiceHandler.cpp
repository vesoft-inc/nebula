...
GeneralStorageServiceHandler::GeneralStorageServiceHandler(StorageEnv* env) : env_(env) {
  kPutCounters.init("put");
  kGetCounters.init("get");
  kRemoveCounters.init("remove");
}

folly::Future<cpp2::ExecResponse> GeneralStorageServiceHandler::future_put(
    const cpp2::KVPutRequest& req) {
  auto* processor = PutProcessor::instance(env_);
  RETURN_FUTURE(processor);
}

folly::Future<cpp2::KVGetResponse> GeneralStorageServiceHandler::future_get(
    const cpp2::KVGetRequest& req) {
  auto* processor = GetProcessor::instance(env_);
  RETURN_FUTURE(processor);
}

folly::Future<cpp2::ExecResponse> GeneralStorageServiceHandler::future_remove(
