    ...
    for (auto& ec : edgeContext_) {
        ...
        if (!props.empty()) {
            for (auto& prop : props) {
                ...
                cols.emplace_back(columnDef(std::move(prop.prop_.name), prop.type_.type));
            }
        }
    }
    if (!cols.empty()) {
