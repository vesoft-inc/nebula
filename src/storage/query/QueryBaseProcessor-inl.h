...
nebula::cpp2::ErrorCode QueryBaseProcessor<REQ, RESP>::handleVertexProps(
  ...
  for (auto& vertexProp : vertexProps) {
    ...
    if (!(*vertexProp.props_ref()).empty()) {
      ...
      // if the list of property names is empty, then all properties on the
      // given tag will be returned
...
template <typename REQ, typename RESP>
nebula::cpp2::ErrorCode QueryBaseProcessor<REQ, RESP>::handleEdgeProps(
  ...
  for (auto& edgeProp : edgeProps) {
    ...
    if (!(*edgeProp.props_ref()).empty()) {
      ...
      // if the list of property names is empty, then all properties on the
      // given edgeType will be returned
...
nebula::cpp2::ErrorCode QueryBaseProcessor<REQ, RESP>::buildFilter(
  ...
  if (!filterStr->empty()) {
    ...
    return checkExp(filter_, false, true, false);
  }
  return nebula::cpp2::ErrorCode::SUCCEEDED;
}
...
nebula::cpp2::ErrorCode QueryBaseProcessor<REQ, RESP>::checkExp(const Expression* exp,
  ...
  switch (exp->kind()) {
    ...
    case Expression::Kind::kSrcProperty: {
      ...
      // We don't check if the property exists intended, because in openCypher, property not exists
      // will be regarded as null
      addPropContextIfNotExists(tagContext_.propContexts_,
                                tagContext_.indexMap_,
                                tagContext_.tagNames_,
                                tagId,
    ...
    case Expression::Kind::kEdgeProperty: {
      ...
      if (exp->kind() == Expression::Kind::kEdgeProperty) {
        ...
        // We don't check if the property exists intended, because in openCypher, property not
        // exists will be regarded as null
      }
      /*
      Because we can't distinguish the edgeType should be positive or negative
      by edge name, as for expression of edge properties, we need extra
      information to decide. So when checkExp is called, user need to make sure
      that related contexts have been set correctly, which are propContexts_,
      indexMap_, edgeNames_ in EdgeContext.
      */
