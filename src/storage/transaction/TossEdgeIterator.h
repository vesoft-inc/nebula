...
class TossEdgeIterator : public SingleEdgeIterator {
 public:
  ...
  bool valid() const override {
    ...
    if (FLAGS_trace_toss) {
      ...
      LOG(INFO) << "TossEdgeIterator::valid() = " << ret << ", key=" << folly::hexlify(key)
                << ", val=" << val;
    }
    return ret;
  }
  ...
  void next() override {
    ...
    while (iter_->valid()) {
      ...
      SCOPE_EXIT { calledByCtor_ = false; };
      if (!iter_->valid()) {
        break;
      }
      if (isEdge(iter_->key())) {
        ...
        if (isLatestEdge(iter_->key()) && setReader(iter_->val())) {
          ...
          lastDstId_ = NebulaKeyUtils::getDstId(context_->vIdLen(), iter_->key()).str();
          lastIsLock_ = false;
          LOG_IF(INFO, FLAGS_trace_toss)
          ...
          LOG_IF(INFO, FLAGS_trace_toss)
              << "edge, hex=" << TransactionUtils::hexEdgeId(context_->vIdLen(), iter_->key())
              << ", is not latest";
        }
        ...
        resumeTasks_.emplace_back(context_->env()->txnMan_->resumeTransaction(
            context_->vIdLen(), context_->spaceId(), iter_->key().str(), recoverEdges_.back()));
        lastRank_ = NebulaKeyUtils::getRank(context_->vIdLen(), rawKey);
        lastDstId_ = NebulaKeyUtils::getDstId(context_->vIdLen(), rawKey).str();
        lastIsLock_ = true;
        continue;
      } else {
        LOG_IF(INFO, FLAGS_trace_toss) << "next to a weird record: \n"
 ...
 private:
  // using shared_ptr because this Iterator may be deleted if there is a limit
  // in nGQL using folly::Synchronized because the scan thread and the resume
  // thread may write same ptr
