...
StatusOr<SubPlan> LookupPlanner::transform(AstContext* astCtx) {
  ...
  SubPlan plan;
  if (lookupCtx->isVectorIndex) {
    auto expr = static_cast<VectorQueryExpression*>(lookupCtx->vectorExpr);
    auto vectorIndexScan =
        VectorIndexScan::make(qctx, expr, lookupCtx->isEdge, lookupCtx->schemaId);
    plan.tail = vectorIndexScan;
    plan.root = vectorIndexScan;

    if (lookupCtx->hasScore) {
      vectorIndexScan->setColNames({kIdColName, kScore});

      auto argument = Argument::make(qctx, kIdColName);
      argument->setInputVertexRequired(false);
      plan.root = argument;
    } else {
      vectorIndexScan->setColNames({kIdColName});
    }
    ...
    if (lookupCtx->hasScore) {
      plan.root = HashInnerJoin::make(
          qctx, vectorIndexScan, plan.root, std::move(hashKeys), std::move(probeKeys));
    }
  } else if (lookupCtx->isFulltextIndex) {
