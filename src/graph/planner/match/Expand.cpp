...
std::unique_ptr<std::vector<storage::cpp2::EdgeProp>> Expand::genEdgeProps(const EdgeInfo& edge) {
  ...
  for (auto edgeType : edge.edgeTypes) {
    auto edgeSchema = matchCtx_->qctx->schemaMng()->getEdgeSchema(matchCtx_->space.id, edgeType);
...
Status Expand::expandSteps(const NodeInfo& node, const EdgeInfo& edge, SubPlan* plan) {
  ...
  if (minHop == 0) {
    ...
    NG_RETURN_IF_ERROR(MatchSolver::appendFetchVertexPlan(
        node.filter, matchCtx_->space, matchCtx_->qctx, &initialExpr_, inputVar_, subplan));
  ...
  // Unionize the results of each expansion which are stored in the firstStep
  // node
...
Status Expand::expandStep(const EdgeInfo& edge,
  ...
  // Extract dst vid from input project node which output dataset format is:
  // [v1,e1,...,vn,en]
  SubPlan curr;
  curr.root = dep;
...
// loopSteps{startIndex} <= maxHop &&  ($lastStepResult == empty ||
// size($lastStepResult) != 0)
