...
void QueryInstance::execute() {
  ...
  // finish query if current sentence is explain
  ...
  scheduler_->schedule()
      ...
      .thenError([this](exception_wrapper &&e) {
        auto err = e.get_exception<ExecutionError>();
        if (err != nullptr) {
          onError(err->status());
        } else {
          onError(Status::Error("%s", e.what().c_str()));
        }
      });
}
...
Status QueryInstance::validateAndOptimize() {
  ...
  // Add space level metrics
  size_t num = 1u;
  if (sentence_->kind() == Sentence::Kind::kSequential) {
    num = static_cast<const SequentialSentences *>(sentence_.get())->numSentences();
  }
  stats::StatsManager::addValue(kNumSentences, num);
  if (FLAGS_enable_space_level_metrics && !spaceName.empty()) {
