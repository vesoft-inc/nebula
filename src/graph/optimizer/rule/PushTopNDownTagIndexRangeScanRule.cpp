...
#include "graph/optimizer/rule/PushTopNDownTagIndexRangeScanRule.h"
...
StatusOr<OptRule::TransformResult> PushTopNDownTagIndexRangeScanRule::transform(
  ...
  for (auto yieldColumn : yieldColumns->columns()) {
    LOG(INFO) << "yieldColumn->expr()->kind()=" << yieldColumn->expr()->kind();
  ...
  auto newIndexScan = static_cast<TagIndexRangeScan *>(indexScan->clone());
