...
#include "common/expression/SubscriptExpression.h"     // for SubscriptExpre...
#include "common/expression/UnaryExpression.h"         // for UnaryExpression
#include "common/expression/VariableExpression.h"      // for VariableExpres...
#include "graph/visitor/FoldConstantExprVisitor.h"     // for FoldConstantEx...
...
TEST_F(FoldConstantExprVisitorTest, TestLogicalExpr) {
  {
    ...
    auto expr = andExpr(constantExpr(false),
...
TEST_F(FoldConstantExprVisitorTest, TestSubscriptExpr) {
  ...
  auto expr = addExpr(
      constantExpr(1),
      subExpr(listExpr({constantExpr(1),
...
TEST_F(FoldConstantExprVisitorTest, TestListExpr) {
  ...
  auto expr = listExpr({addExpr(constantExpr(3), constantExpr(4)),
                        fnExpr("pow", {constantExpr(2), addExpr(constantExpr(2), constantExpr(1))}),
                        constantExpr(2)});
...
TEST_F(FoldConstantExprVisitorTest, TestSetExpr) {
  ...
  auto expr = setExpr({fnExpr("sqrt", {minusExpr(constantExpr(19), constantExpr(3))}),
...
TEST_F(FoldConstantExprVisitorTest, TestMapExpr) {
  // {"jack":1, "tom":pow(2, 2+1), "jerry":5-1} => {"jack":1, "tom":8,
  // "jerry":4}
  auto expr =
      mapExpr({{"jack", constantExpr(1)},
...
TEST_F(FoldConstantExprVisitorTest, TestCaseExpr) {
  ...
  auto expr = caseExpr(fnExpr("pow", {constantExpr(2), addExpr(constantExpr(2), constantExpr(1))}),
