...
// Keep the constraint :
//    Put any vid in result VidPattern.vids to id(V) make the expression
//    evaluate to TRUE!(Ignore not related expression component e.g. attribute
//    expr, so the filter still need eval when filter final result) and non any
...
class VidExtractVisitor final : public ExprVisitor {
 public:
  ...
  static VidPattern intersect(VidPattern &&left, std::pair<std::string, VidPattern::Vids> &&right);
