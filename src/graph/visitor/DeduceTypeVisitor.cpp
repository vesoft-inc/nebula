...
void DeduceTypeVisitor::visit(SubscriptExpression *expr) {
  ...
  auto notValidMapCandidate = (leftMapCandidate & leftType) && !(rightMapSubCandidate & rightType);
...
void DeduceTypeVisitor::visit(EdgePropertyExpression *expr) {
  ...
  if (!schema) {
    status_ =
        Status::SemanticError("`%s', not found edge `%s'.", expr->toString().c_str(), edge.c_str());
    return;
  }
