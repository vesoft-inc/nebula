...
folly::Future<Status> AddGroupExecutor::execute() {
  ...
  return qctx()
      ->getMetaClient()
      ->addGroup(agNode->groupName(), agNode->zoneNames())
      .via(runner())
...
folly::Future<Status> AddZoneIntoGroupExecutor::execute() {
  ...
  return qctx()
      ->getMetaClient()
      ->addZoneIntoGroup(azNode->zoneName(), azNode->groupName())
      .via(runner())
      .thenValue([](StatusOr<bool> resp) {
        if (!resp.ok()) {
...
folly::Future<Status> DropZoneFromGroupExecutor::execute() {
  ...
  return qctx()
      ->getMetaClient()
      ->dropZoneFromGroup(dzNode->zoneName(), dzNode->groupName())
      .via(runner())
      .thenValue([](StatusOr<bool> resp) {
        if (!resp.ok()) {
...
folly::Future<Status> ListGroupsExecutor::execute() {
  SCOPED_TIMER(&execTime_);
  return qctx()->getMetaClient()->listGroups().via(runner()).thenValue(
      [this](StatusOr<std::vector<meta::cpp2::Group>> resp) {
        if (!resp.ok()) {
