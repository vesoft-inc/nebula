...
TEST_F(MutateValidatorTest, InsertEdgeTest) {
  ...
  {
    auto cmd =
        "INSERT EDGE like(start, end) VALUES hash($-,name)->\"Tom\":(2010, "
        "2020);";
    ASSERT_FALSE(checkResult(cmd, {}));
  }
  // vid use function call
...
TEST_F(MutateValidatorTest, UpdateVertexTest) {
  ...
  {
    auto cmd =
        "UPDATE VERTEX \"Tom\" SET person.count = 1, student.age = "
        "$^.student.age + 1";
    ASSERT_FALSE(checkResult(cmd, {}));
  }
  // wrong expr
