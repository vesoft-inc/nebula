...
TEST_F(SchemaTest, addSchemaFromSpaceTest) {
    auto client = gEnv->getClient();
    ASSERT_NE(nullptr, client);
    // prepare
    cpp2::ExecutionResponse resp;
    ...
    // add schemas into empty space
    {
        cmd = "CREATE SPACE empty_space(partition_num=9, replica_factor=1)";
        code = client->execute(cmd, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
        ...
        cmd = "ADD SCHEMA FROM schema_space;";
        code = client->execute(cmd, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    ...
    // add schemas into the same name space
    {
        cmd = "USE schema_space";
        code = client->execute(cmd, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    ...
    // add schemas into space which has existent tag
    {
        cmd = "ADD SCHEMA FROM schema_space;";
        code = client->execute(cmd, resp);
        ASSERT_EQ(cpp2::ErrorCode::E_EXECUTION_ERROR, code);
    }
    // add schemas into not empty space
    {
    ...
    {
        cmd = "DROP SPACE space_9; DROP SPACE empty_space; DROP SPACE schema_space;";
        code = client->execute(cmd, resp);
        ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, code);
    }
}
