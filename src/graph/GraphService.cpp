...
Status GraphService::init() {
    ...
    auto status = executionEngine_->init(ioExecutor_);
    if (!status.ok()) {
        LOG(ERROR) << "ExecutionEngine init failed : "<< status;
        return Status::Error("GraphService init failed");
    }
    return Status::OK();
}
