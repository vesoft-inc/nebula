...
TEST_F(FunctionManagerTest, functionCall) {
  ...
  {
    TEST_FUNCTION(extract, std::vector<Value>({"Hello Nebula Graph", "\\w+"}),
                  List({"Hello", "Nebula", "Graph"}));
    TEST_FUNCTION(extract, std::vector<Value>({"Hello Nebula Graph", "(Nebula|Graph)"}),
                  List({"Nebula", "Graph"}));
...
TEST_F(FunctionManagerTest, time) {
  ...
  {
    // 2021 is not leap
    TEST_FUNCTION(date, {Map({{"year", 2021}, {"month", 2}, {"day", 29}})}, Value::kNullBadData);
  }
  ...
  // year
  {
    TEST_FUNCTION(date, {Map({{"year", -32769}, {"month", 12}, {"day", 15}})}, Value::kNullBadData);
  }
  {
    TEST_FUNCTION(date, {Map({{"year", 32768}, {"month", 12}, {"day", 31}})}, Value::kNullBadData);
  }
  // month
  {
    TEST_FUNCTION(date, {Map({{"year", -32768}, {"month", 13}, {"day", 15}})}, Value::kNullBadData);
  }
  { TEST_FUNCTION(date, {Map({{"year", 32767}, {"month", 0}, {"day", 31}})}, Value::kNullBadData); }
  // day
  {
    TEST_FUNCTION(date, {Map({{"year", -32768}, {"month", 11}, {"day", 0}})}, Value::kNullBadData);
  }
  { TEST_FUNCTION(date, {Map({{"year", 32767}, {"month", 1}, {"day", 32}})}, Value::kNullBadData); }
  // time
  ...
  // hour
  {
    TEST_FUNCTION(time, {Map({{"hour", -1}, {"minute", 9}, {"second", 15}})}, Value::kNullBadData);
  }
  {
    TEST_FUNCTION(time, {Map({{"hour", 24}, {"minute", 9}, {"second", 15}})}, Value::kNullBadData);
  }
  // minute
  {
    TEST_FUNCTION(time, {Map({{"hour", 23}, {"minute", -1}, {"second", 15}})}, Value::kNullBadData);
  }
  {
    TEST_FUNCTION(time, {Map({{"hour", 23}, {"minute", 60}, {"second", 15}})}, Value::kNullBadData);
  }
  // second
  {
    TEST_FUNCTION(time, {Map({{"hour", 23}, {"minute", 59}, {"second", -1}})}, Value::kNullBadData);
  }
  {
    TEST_FUNCTION(time, {Map({{"hour", 23}, {"minute", 59}, {"second", 60}})}, Value::kNullBadData);
  }
  // datetime
  ...
  }
  {
    ...
    for (auto i = 1; i < 4; ++i) {
      path.steps.emplace_back(
          Step(Vertex(folly::to<std::string>(i), {}), 1, "like", 0, {{"likeness", (i + 50)}}));
    }
    args[0] = path;
    for (auto i = 0; i < 3; ++i) {
...
TEST_F(FunctionManagerTest, DataSetRowCol) {
  ...
  // out of range
  {
    TEST_FUNCTION(dataSetRowCol,
                  ...
                  Value::kNullBadData);
    TEST_FUNCTION(
        dataSetRowCol, std::vector<Value>({datasetValue, Value(4), Value(2)}), Value::kNullBadData);
    TEST_FUNCTION(dataSetRowCol,
                  std::vector<Value>({datasetValue, Value(0), Value(-1)}),
                  Value::kNullBadData);
    TEST_FUNCTION(
        dataSetRowCol, std::vector<Value>({datasetValue, Value(0), Value(3)}), Value::kNullBadData);
  }
  // ok
  {
    TEST_FUNCTION(dataSetRowCol, std::vector<Value>({datasetValue, Value(0), Value(0)}), Value(1));
    TEST_FUNCTION(
