...
class NebulaKeyUtils final {
 public:
  ...
  static std::string vectorVertexKey(
      size_t vIdLen, PartitionID partId, const VertexID& vId, PropID propId, char pad = '\0');
  ...
  static std::string vectorVertexPrefix(PartitionID partId);
  ...
  static bool isVectorVertex(const folly::StringPiece& rawKey) {
    constexpr int32_t len = static_cast<int32_t>(sizeof(NebulaKeyType));
    auto type = readInt<uint32_t>(rawKey.data(), len) & kTypeMaskWithVector;
    return static_cast<NebulaKeyType>(type) == NebulaKeyType::kVertex;
  }
