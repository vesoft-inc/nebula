...
#include "common/base/Logging.h"         // for SetStderrLogging, DLOG, LogM...
#include "common/base/Status.h"          // for operator<<, Status
#include "common/base/StatusOr.h"        // for StatusOr
#include "common/datatypes/Date.h"       // for DateTime, Time, Date
...
TEST(Time, secondsTimeConversion) {
  // DateTime
  {
    ...
    for (std::size_t i = 0; i < 4; ++i) {
      values.emplace_back(DateTime(folly::Random::rand32(1000, std::numeric_limits<int16_t>::max()),
  ...
  // Date
  {
    ...
    for (std::size_t i = 0; i < 4; ++i) {
      values.emplace_back(Date(folly::Random::rand32(1000, std::numeric_limits<int16_t>::max()),
                               folly::Random::rand32(1, 13),
                               folly::Random::rand32(1, 29)));
    }
  ...
  }
  {
    std::vector<std::pair<Date, int64_t>> dateSeconds{
        {{Date(1930, 3, 4), -1256947200}, {Date(1970, 1, 1), 0}, {Date(2003, 3, 4), 1046736000}}};
  ...
  }
  {
    ...
    for (std::size_t i = 0; i < timeSeconds.size(); ++i) {
      EXPECT_EQ(timeSeconds[i].second, time::TimeConversion::timeToSeconds(timeSeconds[i].first));
  ...
  // Timestamp
  {
    ...
    // correct int
    EXPECT_TRUE(
        time::TimeUtils::toTimestamp(Value(std::numeric_limits<int64_t>::max() / 1000000000)).ok());
    // correct string
