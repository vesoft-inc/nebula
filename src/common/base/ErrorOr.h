 ...
 *
 * vs. StatusOr<>: ErrorOr<> is an alias of EitherOr with a few accessory
 * methods. The left type of the ErrorOr has to be an integral type or enum,
 * while the left type of StatusOr<> is a Status, which is a more complicate
 * object. This is pretty much the only difference between ErrorOr<> and
 * StatusOr<>
 *
 * So in the scenario that the caller only cares about the error code (or result
 * code), ErrorOr<> would be more suitable. If the caller also needs an error
 * message along with the error code from the callee, then StatusOr<> should be
 * used
 */
