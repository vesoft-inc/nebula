...
#include "common/expression/CaseExpression.h"              // for CaseExpres...
...
TEST_F(CaseExpressionTest, CaseExprToString) {
  ...
  }
  {
    ...
    ASSERT_EQ(
        "CASE (3+5) WHEN 7 THEN 1 WHEN 8 THEN 2 WHEN 8 THEN \"jack\" ELSE "
        "false END",
        expr->toString());
  }
  {
  ...
  }
  {
    ...
    expr->setDefault(ConstantExpression::make(&pool, false));
    ASSERT_EQ(
        "CASE WHEN (\"nebula\" STARTS WITH \"nebu\") THEN \"yes\" ELSE false "
        "END",
        expr->toString());
  }
  {
