...
#include "common/expression/PropertyExpression.h"          // for EdgeProper...
#include "common/expression/RelationalExpression.h"        // for Relational...
...
TEST_F(RelationalExpressionTest, RelationIn) {
  {
    auto expr = RelationalExpression::makeIn(
        &pool, ConstantExpression::make(&pool, 1), ConstantExpression::make(&pool, List({1, 2})));
    auto eval = Expression::eval(expr, gExpCtxt);
    EXPECT_EQ(eval.type(), Value::Type::BOOL);
    EXPECT_EQ(eval, true);
  }
  {
    auto expr = RelationalExpression::makeIn(
        &pool, ConstantExpression::make(&pool, 3), ConstantExpression::make(&pool, List({1, 2})));
    auto eval = Expression::eval(expr, gExpCtxt);
    EXPECT_EQ(eval.type(), Value::Type::BOOL);
...
TEST_F(RelationalExpressionTest, NotInList) {
  {
    ...
    auto listExpr = ListExpression::make(&pool, elist);
    auto expr =
        RelationalExpression::makeNotIn(&pool, ConstantExpression::make(&pool, 12345), listExpr);
    auto value = Expression::eval(expr, gExpCtxt);
    ASSERT_TRUE(value.isBool());
    ASSERT_EQ(false, value);
  }
  {
    ...
    auto listExpr = ListExpression::make(&pool, elist);
    auto expr =
        RelationalExpression::makeNotIn(&pool, ConstantExpression::make(&pool, false), listExpr);
    auto value = Expression::eval(expr, gExpCtxt);
    ASSERT_TRUE(value.isBool());
    ASSERT_EQ(true, value);
  }
...
TEST_F(RelationalExpressionTest, NotInSet) {
  {
    ...
    auto setExpr = SetExpression::make(&pool, elist);
    auto expr =
        RelationalExpression::makeNotIn(&pool, ConstantExpression::make(&pool, 12345), setExpr);
    auto value = Expression::eval(expr, gExpCtxt);
    ASSERT_TRUE(value.isBool());
    ASSERT_EQ(false, value);
  }
  {
    ...
    auto setExpr = ListExpression::make(&pool, elist);
    auto expr =
        RelationalExpression::makeNotIn(&pool, ConstantExpression::make(&pool, false), setExpr);
    auto value = Expression::eval(expr, gExpCtxt);
    ASSERT_TRUE(value.isBool());
    ASSERT_EQ(true, value);
  }
...
TEST_F(RelationalExpressionTest, NotInMap) {
  {
    ...
    auto mapExpr = MapExpression::make(&pool, items);
    auto expr =
        RelationalExpression::makeNotIn(&pool, ConstantExpression::make(&pool, "key1"), mapExpr);
    auto value = Expression::eval(expr, gExpCtxt);
    ASSERT_TRUE(value.isBool());
    ASSERT_EQ(false, value);
  }
  {
    ...
    auto mapExpr = MapExpression::make(&pool, items);
    auto expr =
        RelationalExpression::makeNotIn(&pool, ConstantExpression::make(&pool, "key5"), mapExpr);
    auto value = Expression::eval(expr, gExpCtxt);
    ASSERT_TRUE(value.isBool());
    ASSERT_EQ(true, value);
  }
  {
    ...
    auto mapExpr = MapExpression::make(&pool, items);
    auto expr =
        RelationalExpression::makeNotIn(&pool, ConstantExpression::make(&pool, 12345), mapExpr);
    auto value = Expression::eval(expr, gExpCtxt);
    ASSERT_TRUE(value.isBool());
    ASSERT_EQ(true, value);
  }
...
TEST_F(RelationalExpressionTest, RelationNotStartsWith) {
  ...
  {
    auto expr =
        RelationalExpression::makeNotStartsWith(&pool,
                                                ConstantExpression::make(&pool, Value::kNullValue),
                                                ConstantExpression::make(&pool, "NULL"));
    auto eval = Expression::eval(expr, gExpCtxt);
    EXPECT_EQ(eval.type(), Value::Type::NULLVALUE);
...
TEST_F(RelationalExpressionTest, RelationEndsWith) {
  ...
  }
  {
    ...
    auto expr = RelationalExpression::makeEndsWith(&pool,
...
TEST_F(RelationalExpressionTest, RelationNotEndsWith) {
  ...
  {
    auto expr =
        RelationalExpression::makeNotEndsWith(&pool,
                                              ConstantExpression::make(&pool, 1234),
                                              ConstantExpression::make(&pool, Value::kNullBadData));
    auto eval = Expression::eval(expr, gExpCtxt);
    EXPECT_EQ(eval.type(), Value::Type::NULLVALUE);
