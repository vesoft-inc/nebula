...
TEST(RowReader2, headerInfo) {
    ...
    auto reader2 = std::make_unique<RowReader>(
    ...
    // With schema version, null flags, and offsets
    char data3[] = {0x60,
                    // Schema Version
                    0x01, static_cast<char>(0xFF), static_cast<char>(0xFF),
                    // Null flags
                    0x00, 0x00, 0x00, 0x00, 0x00,
                    // Offsets
                    0x40, static_cast<char>(0xF0)};
    auto reader3 = std::make_unique<RowReader>(
    ...
    char data4[] = {0x01,
                    ...
                    0x00, 0x00, 0x00, 0x00, 0x00,
                    // Offsets
                    static_cast<char>(0xFF), 0x40, 0x08, static_cast<char>(0xF0)};
    auto reader4 = std::make_unique<RowReader>(
...
TEST(RowReader2, encodedData) {
    ...
    EXPECT_EQ(3, reader->headerLen_);
    EXPECT_EQ(2, reader->nullFlags_.size());

    EitherOr<NullValue, bool> bVal;
    EitherOr<NullValue, int32_t> i32Val;
    EitherOr<NullValue, int64_t> i64Val;
    EitherOr<NullValue, uint64_t> u64Val;
    ...
    EXPECT_EQ(str1, sVal.right().toString());
    sVal = "";
    EXPECT_EQ(ResultType::SUCCEEDED,
    ...
    EXPECT_EQ(0xFFFFFFFF, i32Val.right());
    i32Val = 0;
    EXPECT_EQ(ResultType::SUCCEEDED,
              reader->getInt(3, i64Val));
    EXPECT_EQ(0xFFFFFFFFFFFFFFFFL, i64Val.right());
    i64Val = 0;
    EXPECT_EQ(ResultType::SUCCEEDED,
              reader->getInt("int_col2", u64Val));
    EXPECT_EQ(0xFFFFFFFFFFFFFFFFL, u64Val.right());

    // Col 4
    EXPECT_EQ(ResultType::SUCCEEDED,
              reader->getVid(4, i64Val));
    EXPECT_EQ(0x8877665544332211L, i64Val.right());
    i64Val = 0;
    EXPECT_EQ(ResultType::SUCCEEDED,
    ...
    EXPECT_EQ(str2, sVal.right().toString());
    sVal = "";
    EXPECT_EQ(ResultType::SUCCEEDED,
    ...
    EXPECT_EQ(1551331827, i64Val.right());
    i64Val = 0;
    EXPECT_EQ(ResultType::SUCCEEDED,
