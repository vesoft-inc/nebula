...
class MappingConfigurationTest extends FlatSpec with BeforeAndAfter {
  "an Edge" should "be serializable to and deserializable from json" in {
    // edge w/o properties
    val emptyPropertyEdge = Edge("table1", "edge1", "from_column1", "table1", "to_column1", "table2", Some("dt"), Some("pin2mac"))
    ...
    // edge w/ properties
    val withPropertiesEdge = Edge("table1", "edge1", "from_column1", "table1", "to_column1", "table2", Some("dt"), Some("pin2mac"), Some(Seq(Column("col1", "prop1"), Column("col2", "prop2"))))
  ...
  "a Tag" should "be serializable to and deserializable from json" in {
    ...
    val expectedResult ="""{"table_name":"table1","tag_name":"tag1","primary_key":"column1","date_partition_key":null,"type_partition_key":null}"""
    assert(expectedResult == jsonString)
    ...
    // tag w/ properties
    val withPropertiesTag = Tag("table1", "tag1", "column1", Some("dt"), Some("pin2mac"), Some(Seq(Column("col1", "prop1"), Column("col2", "prop2"))))
  ...
  "a MappingConfiguration" should "be serializable to and deserializable from json" in {
    ...
    val tag1 = Tag("table1", "tag1", "column1", Some("dt"), Some("pin2mac"), Some(Seq(Column("col1", "prop1"), Column("col2", "prop2"))))
    val tag2 = Tag("table2", "tag2", "column2", Some("dt"), Some("pin2qq"), Some(Seq(Column("col1", "prop1"), Column("col2", "prop2"))))
    val tag3 = Tag("table3", "tag3", "column3", Some("dt"), Some("pin2wallet"), Some(Seq(Column("col1", "prop1"), Column("col2", "prop2"))))
  ...
  "a MappingConfiguration" should "be constructed from a configuration file" in {
    ...
    val tag2 = Tag("dmt_risk_graph_idmp_node_s_d", "user_pin", "node", Some("dt"), Some("flag"), Some(Seq(Column("src_pri_value", "src_pri_value", "string"))))

    // edge w/ properties
    val edge1 = Edge("dmt_risk_graph_idmp_edge_s_d", "pin2mac", "from_node", "user_pin", "to_node", "mac", Some("dt"), Some("flag"), Some(Seq(Column("src_pri_value", "src_pri_value", "string"))))
