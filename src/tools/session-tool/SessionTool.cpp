...
class SessionTool {
 public:
  static Status sessionsCount(const char* path, const std::string op, const std::string user, const int limit) {
    ...
    iter->Seek(rocksdb::Slice(prefix));
    {
      if ("count" == op) {
        ...
        while (iter->Valid() && iter->key().starts_with(prefix)) {
          if (!user.empty()) {
            auto session = MetaKeyUtils::parseSessionVal(folly::StringPiece(iter->value().data(), iter->value().size()));
    ...
    {  
      if ("list" == op) {
        ...
        while (iter->Valid() && iter->key().starts_with(prefix)) {
          auto sessionID = MetaKeyUtils::getSessionId(folly::StringPiece(iter->key().data(), iter->key().size()));
          auto session = MetaKeyUtils::parseSessionVal(folly::StringPiece(iter->value().data(), iter->value().size()));
          std::string userName = session.get_user_name();
          if (!user.empty()) {
            if (userName == user) {
              LOG(INFO) << sessionID << "\t" 
                        << userName << "\t"
                        << session.get_space_name() << "\t"
                        << session.get_create_time() << "\t"
                        << session.get_update_time() << "\t"
  ...
  static Status deleteSession(const char* path, const std::string user) {
    ...
    }
    {
      ...
      if (user.empty()) {
        ...
        db->DeleteRange(rocksdb::WriteOptions(), db->DefaultColumnFamily(), rocksdb::Slice(prefix), rocksdb::Slice(prefix1));
  ...
  static std::string getUsage() {
    ...
    usage += "-path    [require] the meta rocksdb path, absolute path need to end with path/nebula/0/data\n";
    usage += "-operator    [require] operator type, including count、delete、list and groupCount\n";
    usage += "-user    use name, it can be used with list/delete operator to get/delete sessions of the specificed user \n";
...
int main(int argc, char* argv[]) {
  ...
  if (FLAGS_operator.empty() || ("count" != FLAGS_operator && "delete" != FLAGS_operator
    ...
    LOG(INFO) << "Please specify the operator type, operator type includes count/delete/list/groupCount";
    std::cout << nebula::meta::SessionTool::getUsage() << std::endl;
    return -1;
  }
  auto status = nebula::Status::OK();
  if ("count" == FLAGS_operator || "list" == FLAGS_operator || "groupCount" == FLAGS_operator) {
    status = nebula::meta::SessionTool::sessionsCount(FLAGS_path.c_str(), FLAGS_operator, FLAGS_user, FLAGS_limit);
  } else {
