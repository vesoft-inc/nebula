...
#include "clients/storage/StorageClient.h"      // for Stora...
...
#include "common/network/NetworkUtils.h"        // for Netwo...
#include "common/thrift/ThriftTypes.h"          // for Graph...
#include "interface/gen-cpp2/storage_types.h"   // for KVGet...
...
class SimpleKVVerifyTool {
 public:
  int32_t init() {
    ...
    auto ioExecutor =
        std::make_shared<folly::IOThreadPoolExecutor>(FLAGS_io_threads, std::move(threadFactory));
  ...
  void runInsert(std::unordered_map<std::string, std::string>& data) {
    ...
    if (!resp.failedParts().empty()) {
      for (const auto& partEntry : resp.failedParts()) {
        LOG(ERROR) << "Put Failed in " << partEntry.first
                   << ", Code: " << apache::thrift::util::enumNameSafe(partEntry.second);
      }
      return;
    }
