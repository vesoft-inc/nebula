...
class WalFileInfo final {
 public:
  ...
  LogID firstId() const { return firstLogId_; }

  LogID lastId() const { return lastLogId_; }
  void setLastId(LogID id) { lastLogId_ = id; }

  TermID lastTerm() const { return lastLogTerm_; }
  void setLastTerm(TermID term) { lastLogTerm_ = term; }

  time_t mtime() const { return mtime_; }
  void setMTime(time_t time) { mtime_ = time; }
