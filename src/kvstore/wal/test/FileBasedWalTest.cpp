...
TEST(FileBasedWal, AppendLogs) {
  ...
  for (int i = 1; i <= 10; i++) {
    EXPECT_TRUE(wal->appendLog(
        i /*id*/, 1 /*term*/, 0 /*cluster*/, folly::stringPrintf("Test string %02d", i)));
  }
  ...
  while (it->valid()) {
    ...
    EXPECT_EQ(folly::stringPrintf("Test string %02ld", id), it->logMsg());
    ++(*it);
    ++id;
  }
  EXPECT_EQ(11, id);
...
TEST(FileBasedWal, CacheOverflow) {
  ...
  for (int i = 1; i <= 10000; i++) {
    ASSERT_TRUE(
        wal->appendLog(i /*id*/, 1 /*term*/, 0 /*cluster*/, folly::stringPrintf(kLongMsg, i)));
  }
...
TEST(FileBasedWal, Rollback) {
  ...
  for (int i = 5001; i <= 6000; i++) {
    ASSERT_TRUE(wal->appendLog(
        i /*id*/, 1 /*term*/, 0 /*cluster*/, folly::stringPrintf(kLongMsg, i + 1000)));
  }
...
TEST(FileBasedWal, TTLTest) {
  ...
  ASSERT_EQ(numFilesAfterGC, totalFilesNum - expiredFilesNum)
      << "total " << totalFilesNum << ", numFilesAfterGC " << numFilesAfterGC
...
TEST(FileBasedWal, LinkTest) {
  ...
  EXPECT_TRUE(
      wal->appendLog(1001 /*id*/, 1 /*term*/, 0 /*cluster*/, folly::stringPrintf(kLongMsg, 1001)));
