...
public:
    ...
    void reset() {
        ...
        wal()->reset();
        ResultCode ret_code = engine_->remove(NebulaKeyUtils::systemCommitKey(partId_));
        if (ret_code != ResultCode::SUCCEEDED) {
            LOG(WARNING) << idStr_ << "Remove the committedLogId failed, ErrorCode "
...
private:
    ...
    void cleanup() override {
        ...
        auto ret_code = putCommitMsg(batch.get(), 0, 0);
