...
/**
 * Check whether spaceId and partId exist or not.
 **/
...
/**
 * Check spaceId, partId and return related KV engine.
 * */
...
void NebulaStore::addPart(GraphSpaceID spaceId, PartitionID partId) {
    folly::RWSpinLock::WriteHolder wh(&lock_);
    auto spaceIt = this->spaces_.find(spaceId);
    CHECK(spaceIt != this->spaces_.end()) << "Space does not exist!";
