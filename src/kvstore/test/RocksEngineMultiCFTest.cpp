...
const char kVectorCF[] = "vector";
const char kDefaultCF[] = "default";
...
TEST_P(RocksEngineMultiCFTest, SimpleTest) {
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->put(kVectorCF, "key", "val"));
  std::string val;
  if (flush_) {
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->flush({kVectorCF}));
  }
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->get(kVectorCF, "key", &val));
  EXPECT_EQ("val", val);
  EXPECT_EQ(nebula::cpp2::ErrorCode::E_KEY_NOT_FOUND, engine->get(kDefaultCF, "val", &val));
}
...
TEST_P(RocksEngineMultiCFTest, RangeTest) {
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->multiPut(kVectorCF, std::move(data)));
  if (flush_) {
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->flush({kVectorCF}));
  }
  ...
  auto checkRange = [&](int32_t start, int32_t end, int32_t expectedFrom, int32_t expectedTotal) {
    ...
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->range(kVectorCF, s, e, &iter));
    int num = 0;
    while (iter->valid()) {
      num++;
...
TEST_P(RocksEngineMultiCFTest, PrefixTest) {
  ...
  auto checkPrefix = [&](const std::string& prefix, int32_t expectedFrom, int32_t expectedTotal) {
    ...
    std::unique_ptr<KVIterator> iter;
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->prefix(kVectorCF, prefix, &iter));
    int num = 0;
    while (iter->valid()) {
      num++;
      auto key = iter->key();
...
TEST_P(RocksEngineMultiCFTest, RemoveTest) {
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->get(kVectorCF, "key", &val));
  EXPECT_EQ("val", val);
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->remove(kVectorCF, "key"));
  EXPECT_EQ(nebula::cpp2::ErrorCode::E_KEY_NOT_FOUND, engine->get(kVectorCF, "key", &val));
}
...
TEST_P(RocksEngineMultiCFTest, RemoveRangeTest) {
  ...
  for (int32_t i = 0; i < 100; i++) {
    ...
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->put(kVectorCF, key, value));
    std::string val;
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->get(kVectorCF, key, &val));
    EXPECT_EQ(value, val);
  }
  ...
  }
  {
    ...
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->range(kVectorCF, start, end, &iter));
    int num = 0;
    int expectedFrom = 50;
    while (iter->valid()) {
      num++;
...
TEST_P(RocksEngineMultiCFTest, OptionTest) {
  ...
  // Test CF option
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED,
            engine->setOption(kVectorCF, "disable_auto_compactions", "true"));
  EXPECT_EQ(nebula::cpp2::ErrorCode::E_INVALID_PARM,
            engine->setOption(kVectorCF, "disable_auto_compactions_", "true"));
  EXPECT_EQ(nebula::cpp2::ErrorCode::E_INVALID_PARM,
            engine->setOption(kVectorCF, "disable_auto_compactions", "bad_value"));
...
TEST_P(RocksEngineMultiCFTest, CompactTest) {
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->multiPut(kVectorCF, std::move(data)));
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->compact(kVectorCF));
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->compact(kVectorCF));  // Compact all
}
...
TEST_P(RocksEngineMultiCFTest, IngestTest) {
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->ingest(kVectorCF, files));
  if (flush_) {
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->flush({kVectorCF}));
  }

  std::string result;
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->get(kVectorCF, "key", &result));
  EXPECT_EQ("value", result);
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->get(kVectorCF, "key_empty", &result));
  EXPECT_EQ("", result);
  EXPECT_EQ(nebula::cpp2::ErrorCode::E_KEY_NOT_FOUND,
            engine->get(kVectorCF, "key_not_exist", &result));

  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->ingest(kVectorCF, {}));
}
...
TEST_P(RocksEngineMultiCFTest, BackupRestoreTable) {
  ...
  auto parts = engine->backupTable(kVectorCF, "backup_test", partPrefix, nullptr);
  EXPECT_TRUE(ok(parts));
  sst_files.emplace_back(value(parts));
  auto tags =
      engine->backupTable(kVectorCF, "backup_test", tagsPrefix, [](const folly::StringPiece& key) {
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, restore_engine->ingest(kVectorCF, sst_files));
  if (flush_) {
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->flush({kVectorCF}));
  }
  ...
  std::unique_ptr<KVIterator> iter;
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED,
            restore_engine->prefix(kVectorCF, partPrefix, &iter));
  int index = 0;
  while (iter->valid()) {
    auto key = iter->key();
    auto val = iter->val();
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED,
            restore_engine->prefix(kVectorCF, tagsPrefix, &iter));
  index = 1;
  int num = 0;
  while (iter->valid()) {
    auto key = iter->key();
...
TEST_P(RocksEngineMultiCFTest, PrefixBloomTest) {
  ...
  // Put vertex/edge data into kVectorCF
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED,
            engine->multiPut(kVectorCF, std::move(dataCF_data)));

  // Put system data into kDefaultCF
  ...
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED,
            engine->multiPut(kDefaultCF, std::move(defaultCF_data)));

  if (flush_) {
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->flush({kVectorCF}));
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, engine->flush({kDefaultCF}));
  }

  {
    // vertexPrefix(partId, vId) should find data in kVectorCF
    auto checkVertexPrefix = [&](PartitionID partId, const VertexID& vId) {
      ...
      std::unique_ptr<KVIterator> iter;
      auto code = engine->prefix(kVectorCF, prefix, &iter);
  ...
  {
    // vertexPrefix(partId) should find data in kVectorCF
  ...
  {
    // System data is in kDefaultCF
    auto checkSystemCommit = [&](PartitionID partId) {
      std::string value;
      auto code = engine->get(kDefaultCF, NebulaKeyUtils::systemCommitKey(partId), &value);
      EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED, code);
      EXPECT_EQ("123", value);
    };
    checkSystemCommit(1);
...
// 1. Configure bloom filters on kVectorCF during engine creation.
// 2. All writes and reads for the test would target kVectorCF.
// 3. Statistics would be checked on the database-wide object, but would reflect hits/misses
//    from operations on kVectorCF.
...
TEST(RebuildPrefixBloomFilter, RebuildPrefixBloomFilterForCF) {
  ...
  auto writeAndCheckInitialData = [&](std::unique_ptr<RocksEngine>& engine) {
    ...
    std::string val;
    EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED,
              engine->get(kVectorCF, NebulaKeyUtils::tagKey(kDefaultVIdLen, 1, "1", 0), &val));
  };
  ...
  std::unique_ptr<KVIterator> iter;
  EXPECT_EQ(nebula::cpp2::ErrorCode::SUCCEEDED,
            engine->prefix(kVectorCF, NebulaKeyUtils::tagPrefix(kDefaultVIdLen, 1, "1"), &iter));
  int count = 0;
  while (iter->valid()) {
    count++;
    iter->next();
