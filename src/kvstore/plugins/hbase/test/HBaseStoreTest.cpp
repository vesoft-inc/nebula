...
TEST(HBaseStoreTest, SimpleTest) {
  ...
  hbaseStore->asyncMultiPut(
      spaceId, partId, edgeData, [](ResultCode code) { EXPECT_EQ(ResultCode::SUCCEEDED, code); });
  ...
  auto checkPrefix = [&](const std::string& prefix, int32_t expectedFrom, int32_t expectedTotal) {
    LOG(INFO) << "prefix " << prefix << ", expectedFrom " << expectedFrom << ", expectedTotal "
              << expectedTotal;
  ...
  hbaseStore->asyncRemovePrefix(
      spaceId, partId, prefix3, [](ResultCode code) { EXPECT_EQ(ResultCode::SUCCEEDED, code); });

  hbaseStore->asyncMultiRemove(
      spaceId, partId, edgeKeys, [](ResultCode code) { EXPECT_EQ(ResultCode::SUCCEEDED, code); });
