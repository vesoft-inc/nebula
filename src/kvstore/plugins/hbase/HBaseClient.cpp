...
HBaseClient::HBaseClient(const HostAddr& host) {
  clientsMan_ = std::make_shared<thrift::ThriftClientManager<THBaseServiceAsyncClient>>();
...
ResultCode HBaseClient::get(const std::string& tableName, const std::string& rowKey, KVMap& data) {
  TGet tGet;
  tGet.set_row(rowKey);
...
ResultCode HBaseClient::range(const std::string& tableName,
                              ...
                              std::vector<std::pair<std::string, KVMap>>& dataList) {
  // TODO(zhangguoqing) This is a simple implementation that get all results
  // immediately, and in the future, it will use HBaseScanIter to improve
  // performance.
  TScan tScan;
...
ResultCode HBaseClient::remove(const std::string& tableName, const std::string& rowKey) {
  TDelete tDelete;
