...
const size_t kMaxRowKeyLength = sizeof(PartitionID) + sizeof(VertexID) + sizeof(EdgeType) +
                                sizeof(VertexID) + sizeof(EdgeRanking) + sizeof(EdgeVerPlaceHolder);

HBaseStore::HBaseStore(KVOptions options) : options_(std::move(options)) {
...
ResultCode HBaseStore::range(GraphSpaceID spaceId,
  ...
  for (auto& data : dataList) {
    ...
    key.append(reinterpret_cast<const char*>(&kFillPartId), sizeof(PartitionID)).append(rowKey);
