...
const char RocksEngine::kDefaultColumnFamilyName[] = "default";
const char RocksEngine::kVectorColumnFamilyName[] = "vector";
const char RocksEngine::kIdVidMapColumnFamilyName[] = "idvidmap";
/***************************************
 *
...
RocksEngine::RocksEngine(GraphSpaceID spaceId,
  ...
  std::vector<std::string> cfNames = {
      kDefaultColumnFamilyName, kVectorColumnFamilyName, kIdVidMapColumnFamilyName};
  ...
  if (!status.ok()) {
    ...
    for (size_t i = 0; i < cfCount; ++i) {
      if (cfDescriptors[i].name == kDefaultColumnFamilyName) {
        continue;
      }
  ...
  if (!readonly && spaceId_ != kDefaultSpaceId /* only for storage*/) {
    ...
    status = db_->Get(readOptions,
                      cfHandleMap_[kDefaultColumnFamilyName],
...
std::unique_ptr<WriteBatch> RocksEngine::startBatchWrite() {
  return std::make_unique<RocksWriteBatch>(&cfHandleMap_, cfHandleMap_[kDefaultColumnFamilyName]);
}
...
nebula::cpp2::ErrorCode RocksEngine::get(const std::string& key,
  ...
  auto it = cfHandleMap_.find(kDefaultColumnFamilyName);
  if (it == cfHandleMap_.end()) {
    LOG(ERROR) << "Column family " << kDefaultColumnFamilyName << " not found";
    return nebula::cpp2::ErrorCode::E_UNKNOWN;
  }
...
nebula::cpp2::ErrorCode RocksEngine::prefix(const std::string& prefix,
  ...
  if (FLAGS_enable_rocksdb_prefix_filtering && prefix.size() >= extractorLen_) {
    return prefixWithExtractor(kDefaultColumnFamilyName, prefix, snapshot, storageIter);
  } else {
    return prefixWithoutExtractor(kDefaultColumnFamilyName, prefix, snapshot, storageIter);
  }
}
