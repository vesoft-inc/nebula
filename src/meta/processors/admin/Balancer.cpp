...
bool Balancer::balanceParts(BalanceID balanceId,
  ...
  if (innerBalance_) {
    LOG(INFO) << "Source host " << sourceHost << " Target host " << targetHost;
    while (!checkZoneLegal(sourceHost, targetHost)) {
      sortedHosts.pop_back();
      maxPartsHost = sortedHosts.back();
    }

    auto iter = std::find_if(zoneParts_.begin(), zoneParts_.end(), [&](const auto& pair) {
      return sourceHost == pair.first;
    });
  ...
  while (maxPartsHost.second > maxLoad || minPartsHost.second < minLoad) {
    auto& partsFrom = confirmedHostParts[sourceHost];
    auto& partsTo = confirmedHostParts[targetHost];
    std::sort(partsFrom.begin(), partsFrom.end());
    std::sort(partsTo.begin(), partsTo.end());

    LOG(INFO) << sourceHost << ":" << partsFrom.size() << " -> " << targetHost << ":"
              << partsTo.size();
    ...
    for (auto& partId : diff) {
      ...
      if (partsFrom.size() == partsTo.size() + 1 ||
        ...
        VLOG(3) << "No need to move any parts from " << sourceHost << " to " << targetHost;
        break;
      }

      LOG(INFO) << "[space:" << spaceId << ", part:" << partId << "] " << sourceHost << "->"
                << targetHost;
      auto it = std::find(partsFrom.begin(), partsFrom.end(), partId);
      if (it == partsFrom.end()) {
        LOG(ERROR) << "Part " << partId << " not found in partsFrom " << sourceHost;
        return false;
      }

      if (std::find(partsTo.begin(), partsTo.end(), partId) != partsTo.end()) {
        LOG(ERROR) << "Part " << partId << " already existed in partsTo " << targetHost;
        return false;
      }
      ...
      if (dependentOnGroup) {
        // auto& minParts = zoneParts_[targetHost].second;
        // auto& maxParts = zoneParts_[sourceHost].second;
        if (!checkZoneLegal(sourceHost, targetHost)) {
          LOG(INFO) << "sourceHost " << sourceHost << " targetHost " << targetHost
                    << " not same zone";
          for (auto p : zoneParts_[targetHost].second) {
            LOG(INFO) << "Part " << p;
          }
          auto minIt = std::find(
              zoneParts_[targetHost].second.begin(), zoneParts_[targetHost].second.end(), partId);
          if (minIt != zoneParts_[targetHost].second.end()) {
        ...
        for (auto& sh : sourceHosts->second) {
          auto maxIt =
              std::find(zoneParts_[sh].second.begin(), zoneParts_[sh].second.end(), partId);
          if (maxIt == zoneParts_[sh].second.end()) {
            LOG(INFO) << "Part " << partId << " not found on " << sh;
            continue;
          }
          zoneParts_[sh].second.erase(maxIt);
        }
        ...
        for (auto& th : targetHosts->second) {
          zoneParts_[th].second.emplace_back(partId);
        }
      }
      partsFrom.erase(it);
      partsTo.emplace_back(partId);

      tasks.emplace_back(balanceId, spaceId, partId, sourceHost, targetHost, kv_, client_);
      LOG(INFO) << "Part " << partId << " move from " << sourceHost << " to " << targetHost;
      noAction = false;
    }
    ...
    if (innerBalance_) {
      ...
      auto iter =
          std::find_if(zoneParts_.begin(), zoneParts_.end(), [&sourceHost](const auto& pair) {
            return sourceHost == pair.first;
          });
...
ErrorOr<nebula::cpp2::ErrorCode, bool> Balancer::getHostParts(GraphSpaceID spaceId,
  ...
  if (dependentOnGroup && properties.group_name_ref().has_value()) {
    ...
    auto activeHostsRet = ActiveHostsMan::getActiveHostsWithGroup(kv_, spaceId);
    if (!nebula::ok(activeHostsRet)) {
      return nebula::error(activeHostsRet);
    }
...
nebula::cpp2::ErrorCode Balancer::assembleZoneParts(const std::string& groupName,
  ...
  std::unordered_map<HostAddr, std::vector<PartitionID>> zps;
  ...
  for (auto zoneName : zoneNames) {
    ...
    for (const auto& host : hosts) {
      ...
      for (const auto& h : hosts) {
        auto it = hostParts.find(h);
        if (it != hostParts.end()) {
          zps[host].insert(zps[host].end(), it->second.begin(), it->second.end());
        }
      }
    }
  }

  for (auto it = zps.begin(); it != zps.end(); it++) {
    LOG(INFO) << "Host " << it->first;
  ...
  for (auto it = hostParts.begin(); it != hostParts.end(); it++) {
    ...
    for (auto hostIter = hosts.begin(); hostIter != hosts.end(); hostIter++) {
      auto partIter = zps.find(*hostIter);
      LOG(INFO) << "Zone " << name << " have the host " << host;
      if (partIter != zps.end() && !partIter->second.empty()) {
        for (auto p : partIter->second) {
          LOG(INFO) << "part for host " << host << " " << p;
        }
        zoneParts_[host] = ZoneNameAndParts(name, partIter->second);
      } else {
        zoneParts_[host] = ZoneNameAndParts(name, std::vector<PartitionID>());
      }
    }
  }
...
std::vector<std::pair<HostAddr, int32_t>> Balancer::sortedHostsByParts(const HostParts& hostParts) {
  ...
  for (auto hp : hosts) {
    LOG(INFO) << "Host " << hp.first << " Parts " << hp.second;
  }
  return hosts;
}
