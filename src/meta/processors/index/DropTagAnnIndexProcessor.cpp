...
void DropTagAnnIndexProcessor::process(const cpp2::DropTagIndexReq& req) {
  ...
  if (!nebula::ok(tagIndexIDRet)) {
    ...
    if (retCode == nebula::cpp2::ErrorCode::E_INDEX_NOT_FOUND) {
      if (req.get_if_exists()) {
        ...
        LOG(INFO) << "Drop Tag Ann Index Failed, index name " << indexName
  ...
  bool hasEdgeSchema = std::any_of(schemas.begin(), schemas.end(), [](auto& schema) {
    return schema.getType() != nebula::cpp2::SchemaID::Type::tag_id;
  });
  if (hasEdgeSchema) {
  ...
  LOG(INFO) << "Drop Tag Ann Index " << indexName;
