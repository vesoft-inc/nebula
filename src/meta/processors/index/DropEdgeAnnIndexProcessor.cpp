...
void DropEdgeAnnIndexProcessor::process(const cpp2::DropEdgeIndexReq& req) {
  ...
  if (!nebula::ok(edgeIndexIDRet)) {
    ...
    if (retCode == nebula::cpp2::ErrorCode::E_INDEX_NOT_FOUND) {
      if (req.get_if_exists()) {
        ...
        LOG(INFO) << "Drop Edge Ann Index Failed, index name " << indexName
  ...
  if (!nebula::ok(indexItemRet)) {
    ...
    LOG(INFO) << "Drop Edge Ann Index Failed: SpaceID " << spaceID << " Index Name: " << indexName
              << " error: " << apache::thrift::util::enumNameSafe(retCode);
    handleErrorCode(retCode);
    onFinished();
    return;
  ...
  if (hasTagSchema) {
    LOG(INFO) << "Drop Edge Ann Index Failed: Index Name " << indexName << " is not EdgeIndex";
  ...
  LOG(INFO) << "Drop Edge Ann Index " << indexName;
