...
void ListTagAnnIndexesProcessor::process(const cpp2::ListTagIndexesReq& req) {
  ...
  std::vector<cpp2::AnnIndexItem> items;
  ...
  while (indexIndexIter->valid()) {
    ...
    if (!nebula::ok(indexSchemaRet)) {
      ...
      LOG(INFO) << "Get tag index failed, SpaceID: " << space << ", indexId: " << indexId;
      handleErrorCode(retCode);
      onFinished();
      return;
    }
