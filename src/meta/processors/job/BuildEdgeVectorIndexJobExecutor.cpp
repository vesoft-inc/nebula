...
folly::Future<Status> BuildEdgeVectorIndexJobExecutor::executeInternal(
    HostAddr&& address, std::vector<PartitionID>&& parts) {
  folly::Promise<Status> pro;
  auto f = pro.getFuture();
  adminClient_
      ->addTask(cpp2::JobType::BUILD_EDGE_VECTOR_INDEX,
                jobId_,
                taskId_++,
                space_,
                std::move(address),
