...
TEST(ProcessorTest, CreateVectorTagTest) {
  ...
  std::vector<cpp2::ColumnDef> vecCols;
  vecCols.emplace_back(TestUtils::columnDef(3, PropertyType::VECTOR, Value(), false, 2));
  vecCols.emplace_back(TestUtils::columnDef(4, PropertyType::VECTOR, Value(), false, 3));
  schema.columns_ref() = std::move(cols);
  schema.vector_columns_ref() = std::move(vecCols);
  {
    // Space not exist
  ...
  // Wrong default value
  {
    ...
    schemaWithDefault.vector_columns_ref() = std::move(vecColsWithDefault);
