...
TEST(ConfigManTest, MetaConfigManTest) {
  ...
  auto client =
      std::make_shared<MetaClient>(threadPool, std::vector<HostAddr>{HostAddr(localIp, sc->port_)});
  ...
  // mock some test gflags to meta
  {
    ...
    configItems.emplace_back(toThriftConfigItem(module,
                                                "int64_key_immutable",
    ...
    configItems.emplace_back(toThriftConfigItem(module,
                                                ...
                                                cpp2::ConfigType::INT64,
                                                mode,
                                                toThriftValueStr(cpp2::ConfigType::INT64, 101L)));
    configItems.emplace_back(toThriftConfigItem(module,
                                                "bool_key",
                                                cpp2::ConfigType::BOOL,
                                                mode,
                                                toThriftValueStr(cpp2::ConfigType::BOOL, false)));
    configItems.emplace_back(toThriftConfigItem(module,
                                                "double_key",
                                                cpp2::ConfigType::DOUBLE,
                                                mode,
                                                toThriftValueStr(cpp2::ConfigType::DOUBLE, 1.23)));
    std::string defaultValue = "something";
    configItems.emplace_back(
        toThriftConfigItem(module,
                           "string_key",
                           cpp2::ConfigType::STRING,
                           mode,
                           toThriftValueStr(cpp2::ConfigType::STRING, defaultValue)));
    configItems.emplace_back(
        toThriftConfigItem(module,
                           "nested_key",
                           cpp2::ConfigType::NESTED,
                           mode,
                           toThriftValueStr(cpp2::ConfigType::NESTED, FLAGS_nested_key)));
...
TEST(ConfigManTest, MockConfigTest) {
  ...
  // mock one ClientBaseGflagsManager, and do some update value in console,
  // check if it works
  auto threadPool = std::make_shared<folly::IOThreadPoolExecutor>(1);
  IPv4 localIp;
  network::NetworkUtils::ipv4ToInt("127.0.0.1", localIp);
...
TEST(ConfigManTest, RocksdbOptionsTest) {
  ...
  auto metaServerContext =
      meta::TestUtils::mockMetaServer(localMetaPort, metaPath.c_str(), kClusterId);
  ...
  auto mClient = std::make_unique<meta::MetaClient>(threadPool, metaAddr, options);
  ...
  // mock some rocksdb gflags to meta
  {
    ...
    configItems.emplace_back(
        toThriftConfigItem(module,
                           ...
                           type,
                           mode,
                           toThriftValueStr(type, FLAGS_rocksdb_column_family_options)));
    cfgMan.registerGflags(configItems);
  }

  std::string dataPath = folly::stringPrintf("%s/storage", rootPath.path());
  auto sc = storage::TestUtils::mockStorageServer(
      mClient.get(), dataPath.c_str(), localIp, storagePort, true);
  ...
  }
  {
    ...
    std::string rocksPath =
        folly::stringPrintf("%s/disk1/nebula/%d/data", dataPath.c_str(), spaceId);
    rocksdb::Status status = rocksdb::LoadLatestOptions(
        rocksPath, rocksdb::Env::Default(), &loadedDbOpt, &loadedCfDescs);
    ASSERT_TRUE(status.ok());
