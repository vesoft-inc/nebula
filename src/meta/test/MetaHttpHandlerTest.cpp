...
public:
    void SetUp() override {
        ...
        WebService::registerHandler("/get_meta", [] {
...
TEST(MetaHttpHandlerTest, MetaStatusTest) {
    std::string pidFile = FLAGS_meta_pid_file;
    fs::TempFile tmpfile("/tmp/nebula-metad-tmp.pid.XXXXXX");
    FLAGS_meta_pid_file = tmpfile.path();
    FLAGS_load_data_interval_second = 1;
    {
        std::string resp;
        ASSERT_TRUE(getUrl("/get_meta?meta=status", resp));
        ASSERT_EQ(std::string("status=stop\n"), resp);
    }
    {
        std::string resp;
        ASSERT_TRUE(getUrl("/get_meta?meta=status&returnjson", resp));
    ...
    {
        std::string resp;
        ASSERT_TRUE(getUrl("/get_meta123?meta=status", resp));
        ASSERT_TRUE(resp.empty());
    }

    FLAGS_meta_pid_file = pidFile;
}
