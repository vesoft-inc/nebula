...
MetaClient::MetaClient(std::shared_ptr<folly::IOThreadPoolExecutor> ioThreadPool,
    ...
    clientsMan_ =
        std::make_shared<thrift::ThriftClientManager<meta::cpp2::MetaServiceAsyncClient>>();
    updateActive();
    updateLeader();
...
void MetaClient::dnsQUeryThreadFunc() {
    ...
    for (auto it = addrs_.cbegin(); it != addrs_.cend(); ++it) {
        auto addrOpt = network::NetworkUtils::updateHost(*it);
        if (addrOpt == boost::none) {
            continue;
        }
        addrs.emplace_back(addrOpt.value());
...
static TagSchemas __buildTagSchemas(std::vector<cpp2::TagItem> tagItemVec) {
    ...
    for (auto& tagIt : tagItemVec) {
        ...
        for (auto colIt : tagIt.schema.get_columns()) {
            VLOG(3) << "Load TagId: " << tagIt.tag_id << ", prop: " << colIt.name
...
static EdgeSchemas __buildEdgeSchemas(std::vector<cpp2::EdgeItem> edgeItemVec) {
    ...
    for (auto& edgeIt : edgeItemVec) {
        ...
        for (auto colIt : edgeIt.schema.get_columns()) {
            VLOG(3) << "Load EdgeType: " << edgeIt.edge_type << ", prop: " << colIt.name
                    << ", type: " << static_cast<int32_t>(colIt.type.type)
                    << ", default_type: " << colIt.default_value.getType();
            schema->addField(colIt.name, std::move(colIt.type));
            schema->addDefaultValue(colIt.name, std::move(colIt.default_value));
...
std::unordered_map<network::InetAddress, std::vector<PartitionID>> MetaClient::reverse(
                    ...
                    if (t.hasException()) {
                        ...
                        if (retry < retryLimit) {
                            ...
                            stats::Stats::addStatsValue(
                                stats_.get(), false, duration.elapsedInUSec());
...
PartsMap MetaClient::doGetPartsMap(const network::InetAddress& host, const LocalCache& localCache) {
    PartsMap partMap;
...
folly::Future<StatusOr<bool>> MetaClient::multiPut(
    ...
    if (!nebula::meta::MetaCommon::checkSegment(segment) || pairs.empty()) {
...
folly::Future<StatusOr<std::string>> MetaClient::get(std::string segment, std::string key) {
    if (!nebula::meta::MetaCommon::checkSegment(segment) || key.empty()) {
        return Status::Error("arguments invalid!");
    }
    cpp2::GetReq req;
    req.set_segment(std::move(segment));
...
folly::Future<StatusOr<std::vector<std::string>>> MetaClient::multiGet(
    ...
    if (!nebula::meta::MetaCommon::checkSegment(segment) || keys.empty()) {
        return Status::Error("arguments invalid!");
    }
...
folly::Future<StatusOr<std::vector<std::string>>> MetaClient::scan(std::string segment,
                                                                   std::string start,
                                                                   std::string end) {
    if (!nebula::meta::MetaCommon::checkSegment(segment) || start.empty() || end.empty()) {
        return Status::Error("arguments invalid!");
    }
    cpp2::ScanReq req;
    req.set_segment(std::move(segment));
...
Status MetaClient::checkSpaceExistInCache(const network::InetAddress& host, GraphSpaceID spaceId) {
    //    folly::RWSpinLock::ReadHolder holder(localCacheLock_);
    const ThreadLocalInfo& threadLocalInfo = getThreadLocalInfo();
    auto it = threadLocalInfo.localCache_.find(spaceId);
    if (it != threadLocalInfo.localCache_.end()) {
...
folly::Future<StatusOr<nebula::cpp2::Schema>> MetaClient::getTagSchema(int32_t spaceId,
                                                                       std::string name,
                                                                       int64_t version) {
    cpp2::GetTagReq req;
    req.set_space_id(spaceId);
...
folly::Future<StatusOr<nebula::cpp2::Schema>> MetaClient::getEdgeSchema(GraphSpaceID spaceId,
                                                                        std::string name,
                                                                        SchemaVer version) {
    cpp2::GetEdgeReq req;
    req.set_space_id(spaceId);
...
folly::Future<StatusOr<nebula::cpp2::IndexItem>> MetaClient::getTagIndex(GraphSpaceID spaceID,
                                                                         std::string name) {
...
folly::Future<StatusOr<std::vector<nebula::cpp2::IndexItem>>> MetaClient::listTagIndexes(
    GraphSpaceID spaceID) {
...
folly::Future<StatusOr<nebula::cpp2::IndexItem>> MetaClient::getEdgeIndex(GraphSpaceID spaceID,
                                                                          std::string name) {
...
folly::Future<StatusOr<std::vector<nebula::cpp2::IndexItem>>> MetaClient::listEdgeIndexes(
    GraphSpaceID spaceID) {
...
StatusOr<std::shared_ptr<nebula::cpp2::IndexItem>> MetaClient::getTagIndexByNameFromCache(
...
StatusOr<std::shared_ptr<nebula::cpp2::IndexItem>> MetaClient::getEdgeIndexByNameFromCache(
    const GraphSpaceID space,
    const std::string& name) {
    if (!ready_) {
        return Status::Error("Not ready!");
...
StatusOr<std::shared_ptr<nebula::cpp2::IndexItem>> MetaClient::getTagIndexFromCache(
    GraphSpaceID spaceId,
    IndexID indexID) {
    if (!ready_) {
        return Status::Error("Not ready!");
...
StatusOr<std::shared_ptr<nebula::cpp2::IndexItem>> MetaClient::getEdgeIndexFromCache(
    GraphSpaceID spaceId,
    IndexID indexID) {
    if (!ready_) {
        return Status::Error("Not ready!");
...
StatusOr<std::vector<std::shared_ptr<nebula::cpp2::IndexItem>>> MetaClient::getTagIndexesFromCache(
    GraphSpaceID spaceId) {
    if (!ready_) {
        return Status::Error("Not ready!");
    }
...
StatusOr<std::vector<std::shared_ptr<nebula::cpp2::IndexItem>>> MetaClient::getEdgeIndexesFromCache(
    GraphSpaceID spaceId) {
    if (!ready_) {
        return Status::Error("Not ready!");
    }
...
std::vector<nebula::cpp2::RoleItem> MetaClient::getRolesByUserFromCache(const std::string& user) {
...
folly::Future<StatusOr<bool>> MetaClient::grantToUser(nebula::cpp2::RoleItem roleItem) {
...
folly::Future<StatusOr<bool>> MetaClient::revokeFromUser(nebula::cpp2::RoleItem roleItem) {
...
folly::Future<StatusOr<std::map<std::string, std::string>>> MetaClient::listUsers() {
...
folly::Future<StatusOr<std::vector<nebula::cpp2::RoleItem>>> MetaClient::listRoles(
    GraphSpaceID space) {
...
folly::Future<StatusOr<std::vector<nebula::cpp2::RoleItem>>> MetaClient::getUserRoles(
    std::string account) {
...
folly::Future<StatusOr<int64_t>> MetaClient::balance(std::vector<network::InetAddress> hostDel,
    ...
    if (!hostDel.empty()) {
        ...
        std::transform(
            hostDel.begin(), hostDel.end(), std::back_inserter(tHostDel), [](const auto& h) {
...
StatusOr<LeaderMap> MetaClient::loadLeader() {
    ...
    for (auto& item : hostItems) {
        ...
        LOG(INFO) << "Load leader of " << hostAddr << " in " << item.get_leader_parts().size()
                  << " space";
    }
