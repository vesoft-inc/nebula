...
启动多副本 Meta 服务需将多个地址配置到`meta_server_addrs`，地址间需使用逗号隔开。

使用`data_path`设置`Meta`的基础存储路径。
***

#### 启动 Nebula 集群


目前，nebula 集群由`scripts/services.sh`管理，可使用此脚本`start`，`stop`或 `restart`重启集群。
...
`data_path`                 | ""                       | 根数据路径
`peers`                     | ""                       | 一系列由逗号隔开的IP地址，IP数与副本数相等，如果为空，则表明副本数为1
...
`reuse_port`                        | true                       | 开启 SO_REUSEPORT 选项
`data_path`                         | ""                         | 根数据路径，多条路径由逗号隔开，对 rocksdb 引擎，一个路径为一个实例
`local_ip`                          | ""                         |  IP 地址和监听端口用于此服务器
`daemonize`                         | true                       | 作为 daemon 进程运行
...
`rocksdb_db_options`                | ""                         |  DBOptions，每个选项以 <option_name>:<option_value> 格式给出，以.隔开
`rocksdb_column_family_options`     | ""                         | ColumnFamilyOptions，每个选项以 <option_name>:<option_value> 格式给出，以.隔开
`rocksdb_block_based_table_options` | ""                         | BlockBasedTableOptions，每个选项以<option_name>:<option_value> 格式给出，以.隔开
...
`raft_rpc_timeout_ms`               | 500                        | raft 客户端 RPC 超时时长
`accept_log_append_during_pulling`  | false                      |  pull snapshot 过程中不接受新log
`raft_heartbeat_interval_secs`      | 5                          | 每次心跳间隔时长
...
`client_idle_timeout_secs`      | 0                        | 关闭 idle 连接前的时长， 0为 无穷大
`session_idle_timeout_secs`     | 600                      | idle sessions过期时长，0为无穷大
...
`u`                      | ""            | 用于身份验证的用户名
`p`                      | ""            | 用于身份验证的密码
`enable_history`         | false         | 不强制存储命令大历史
`server_conn_timeout_ms` | 1000          | 连接超时时长
