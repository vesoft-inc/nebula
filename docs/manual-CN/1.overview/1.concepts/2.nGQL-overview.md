...
- 简单类型： **vid**, **integer** (int64), **double**, **float**, **bool**, **string**, **path**, **timestamp**, **year**, **month** (year/month), **date**, **datetime**
  - **vid** ： 64 有符号整数，用来表示点 ID
- 简单类型列表，如： **integer[]**, **double[]**, **string[]**
- **Map**: 键值对列表。键类型必须为 **字符**，值类型必须与给定 map
- **对象** (未来版本支持): 键值对列表。键类型必须为**字符**，值可以是任意简单类型
- **元组列表**: *只适用于返回值*。由元数据和数据（多行）组成 。元数据包含列名和类型。
