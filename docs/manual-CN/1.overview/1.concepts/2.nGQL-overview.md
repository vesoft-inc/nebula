...
> 对于大多数读者，如果不熟悉 BNF 可以跳过本节

### 总览

- 整套语法可分为三部分：**查询**，**更改**，**管理**
- 每条语法均可返回一个数据集，第个数据集均包含一个 schema 和多条数据

### 语法组合

- 语法组合有两种方式：
  - 语法可使用管道函数 "**|** 连接，前一条语法返回的结果可作为下一条语法的查询条件
  - 支持使用 "**;**" 批量输入多条语法，批处理时返回最后一条语法结果
...
<span style="color:blue">**WHERE**</span> 语法仅适用于最终返回结果，对中间结果不适用。

跳过 <span style="color:blue">**STEP[S]**</span> 表示 **一步**

从起始点出发一跳，遍历所有满足<span style="color:blue">**WHERE**</span> 语法的关联点，只返回满足 <span style="color:blue">**WHERE**</span> 语法的结果。

多跳查询时，<span style="color:blue">**WHERE**</span> 语法只适用于最终结果，对中间结果不适用。例如：

```SQL
GO 2 STEPS FROM me OVER friend WHERE birthday > "1988/1/1"
```

以上语法查询所有生日在 1988/1/1 之后的二度好友。
...
以上语法查询所有生日在 1988/1/1 之后的好友，最多查询为 3 度。如果只查询到一条，则会全部返回。如果没有满足条件的数据，则查询 2 度好友，如果还是没有则查询 3 度。

以下语法查询生日在 1988/1/1 之后的3 度 至 5 度好友。
...
属性关联很常见，如 <span style="color:blue">**WHERE**</span> 语法和 <span style="color:blue">**YIELD**</span> 语法。nGQL 采用如下方式定义属性关联：
...
All property names start with a letter. 所有属性名以字母开头。个别系统属性以 "\_" 开头。  "\_" 保留值。
