...
In **Nebula Graph** we classify different vertices into one group which is named a tag. The `CREATE TAG` statement defines a tag, with a type name and a property list. The `CREATE EDGE` statement defines an edge type.
...
```ngql
nebula> INSERT VERTEX player(name, age) values 100:("Tim", 42);
...
The following example shows you how to change the value of the `degree` attribute in the **follow** edge between `VID` `100` and `VID` `101`. Now we change the `degree` attribute from `95` to `96`.
...
You can delete any vertex from your graph space.
...
You can delete any edge from your graph space.
