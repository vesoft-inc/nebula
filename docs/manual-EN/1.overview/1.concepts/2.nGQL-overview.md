...
`nGQL` is a declarative, textual query language like SQL, but for graphs. Unlike `SQL`, `nGQL` is all about expressing graph patterns. As `nGQL` is being developed, we will add more features and further simplify the existing ones. Syntax descriptions in this manual might be different from future releases.

## Goals

- Easy to learn
- Easy to understand
- To focus on the online queries, and provide the foundation for the offline computation
...
- **Graph Space** : A physically isolated space for different graphs
- **Tag** : A label associated with a list of properties
  - Each tag has a name (human readable string), and internally each tag is assigned a 32-bit integer
  - Each tag associates with a list of properties, and each property has a name and a type
  - One tag can be dependent on other tags. The dependency is a constraint, for instance, if tag *S* depends on tag *T*, then tag *S* cannot exist unless tag *T* exists
...
- Statements can be composed in two ways:
  - Statements can be piped together using operator "**|**", much like the pipe in the shell scripts. The result yielded from the previous statement can be redirected to the next statement as input
...
- Simple type: **vid**, **double**, **int**, **bool**, **string**, and **timestamp**
...
Nebula supports multiple graph spaces. Data in different graph spaces is physically isolated. Before executing a query, you must select a graph space using the following statement
...
Obviously, you probably guess the meaning of the query is to get all my fof (friend of friend) whose birthday is after 1988/1/1. You are absolutely right. We will not apply the filter to my friends (in the first step)
...
This query tries to find any friend of mine whose birthday is after 1988/1/1. If it finds one, it will return all the result. If it cannot find any, it will check second degree friends to see if anyone's birthday is after 1988/1/1. If no result is returned, it will further look for third degree friends.
