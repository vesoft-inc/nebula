...
if [[ $SOURCE_DIR/CMakeLists.txt -nt $SOURCE_DIR/Makefile ||
    ...
    if !($VGRAPH_CMAKE $CMAKE_FLAGS -DCMAKE_INSTALL_LIBDIR=lib -DSNAPPY_BUILD_TESTS=NO -DCMAKE_C_FLAGS:STRING="$COMPILER_FLAGS" -DCMAKE_CXX_FLAGS:STRING="$COMPILER_FLAGS" -DCMAKE_EXE_LINKER_FLAGS:STRING="$LINKER_FLAGS"    $SOURCE_DIR); then
        cd $CURR_DIR
        echo
